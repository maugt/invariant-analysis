...
**This is write-spec v2.** It is standalone — do not consult write-spec.md (v1). v1 is frozen as the experiment baseline; v2 incorporates lessons from batch-01/02 (VY-209), the hailort floor finding, the PR #155 consumer-enumeration miss, and the VY-212 / VY-211 / VY-213 remediation tickets.
...
**Concrete example of FM-2 (wrong-layer framing) caught by this step**:

> Rough task: *"ax.clear() adds extra ticks on shared axes"*.
> Anchored framing: fix `_AxesBase.clear()`.
> Alternative site: fix `Axis.clear()` which is called by `_AxesBase.clear()`.
> Why alternative is better: `Axis.clear()` is the one that resets tick state; `_AxesBase.clear()` just delegates. The bug is actually in the reset inside `Axis.clear()`. Patching `_AxesBase.clear()` is a workaround one layer up.

Without the alternative-diagnosis pass, the spec would have locked in the wrong layer.
